...
def extract_content_from_page(file_path):
    ...
    parsed_data['date'] = soup.find('article', {'class': 'single-news'}).find('time', {'news-startdate'}).text.strip()

    """print(soup.find('article', {'class': 'single-news'})
          .find('time', {'news-startdate'}).text.strip()
          )"""
    """print(soup.find('article', {'class': 'single-news'}).find('h1', {'single-news__title'}).text.strip())"""
    parsed_data['title'] = soup.find('article', {'class': 'single-news'}).find('h1', {'single-news__title'}).text.strip()
    """print(soup.find('section', {'class': 'single-news__content'}).text.strip())"""
    parsed_data['content'] = soup.find('section', {'class': 'single-news__content'}).text.strip()
    return parsed_data
